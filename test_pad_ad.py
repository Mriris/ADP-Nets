...
from utils.loader import get_test_data, get_test_data_SR
import utils
...
parser.add_argument('--no_gt', action='store_true', default=False, help='only noisy images, no ground truth')
...
if args.no_gt:
    test_dataset = get_test_data_SR(args.input_dir)
else:
...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(test_loader, 0):
        ...
        if (ii + 1) % 50 == 0:
            print(f"Processed {ii + 1}/{len(test_dataset)} images")

if not args.no_gt:
    psnr_val_rgb = sum(psnr_val_rgb)/len(test_dataset)
    print("PSNR: %f " %(psnr_val_rgb))
else:
    print(f"Done! Processed {len(test_dataset)} images, saved to {args.result_dir}")
